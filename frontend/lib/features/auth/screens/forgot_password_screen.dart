...
class ForgotPasswordScreen extends StatefulWidget {
  // Which of this email's accounts to reset ('lawyer', 'client' or
  // 'law_student') — passed from the Login screen's tab this was opened
  // from, since the same email can now back a separate account per role and
  // the backend needs to know which one.
  final String? role;
...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> {
  ...
  Future<void> _sendCode() async {
    ...
    try {
      await DioClient.instance.post('/auth/forgot-password',
          data: {'email': email, 'role': widget.role ?? ''});
      if (!mounted) return;
      setState(() {
        _email = email;
        _step = _Step.otp;
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      await DioClient.instance.post('/auth/forgot-password/verify-otp',
          data: {'email': _email, 'role': widget.role ?? '', 'otp': _otp});
      if (!mounted) return;
      setState(() {
    ...
    return Column(children: [
      ...
      TextButton(
        ...
        child: Text(
            ...
            style: TextStyle(
                color:
                    _resendSeconds > 0 ? AppColors.textMuted : AppColors.gold,
