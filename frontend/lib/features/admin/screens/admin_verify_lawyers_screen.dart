...
class _AdminVerifyLawyersScreenState extends State<AdminVerifyLawyersScreen> {
                  ...
                  children: [
                    ...
                    if (isImage && content.isNotEmpty)
                      ClipRRect(
                        ...
                        child: Image.memory(
                          base64Decode(content.contains(',')
                              ? content.split(',').last
                              : content),
                          fit: BoxFit.contain,
                          height: 360,
                          errorBuilder: (_, __, ___) =>
                              const Text('Could not render document preview.'),
                        ),
                      )
                    else
                      Container(
                        ...
                        decoration: BoxDecoration(
                            color: kAdminBg,
                            borderRadius: BorderRadius.circular(10)),
                        child: Text('Preview not available for $mime',
                      ...
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: () {
                            ...
                            _showVerifyDialog(lawyer,
                                hasDoc: true, reject: true);
                          },
                          icon: const Icon(Icons.close_rounded,
                              color: kAdminRed, size: 16),
  ...
  void _showVerifyDialog(dynamic lawyer,
      {required bool hasDoc, bool reject = false}) {
    if (!hasDoc && !reject) {
      // No document at all — surface that instead of pretending one exists.
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('No verification document uploaded for this lawyer.')));
      return;
    }
          ...
          if (_loading)
            ...
            ..._rows.map((l) => Container(
                  ...
                  child: Row(children: [
                    ...
                    AdminBadge(l['verification_status'] ?? '',
                    ...
                    if (_status == 'pending') ...[
                      ...
                      IconButton(
                        tooltip: 'Reject',
                        icon:
                            const Icon(Icons.cancel_rounded, color: kAdminRed),
                        onPressed: () =>
                            _showVerifyDialog(l, hasDoc: true, reject: true),
                      ),
                    ],
                  ]),
                )),
