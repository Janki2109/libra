...
class _AdminConsultationsScreenState extends State<AdminConsultationsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await _repo.consultations(
          status: _status,
          ...
          child: Column(
              ...
              children: [
                ...
                Row(children: [
                  ...
                  AdminBadge(c['payment_status'] ?? '',
                      AdminBadge.colorFor(c['payment_status'] ?? '')),
                ]),
                const Divider(height: 24),
                _row('Client', '${c['client_name']} (${c['client_email']})'),
                _row('Lawyer', '${c['lawyer_name']} (${c['lawyer_email']})'),
                _row('Date / Time',
                    '${fmtDate(c['consultation_date'])} · ${c['consultation_time']}'),
                ...
                rows: _rows.map((c) {
                  return DataRow(
                    ...
                    cells: [
                      ...
                      DataCell(AdminBadge(c['payment_status'] ?? '',
                          AdminBadge.colorFor(c['payment_status'] ?? ''))),
